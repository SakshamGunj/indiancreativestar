...
export default function AdminCertificates() {
  ...
  // Admin password (same as other admin interfaces)
  ...
  const handleLogin = async () => {
    ...
    if (password === ADMIN_PASSWORD) {
      ...
      sessionStorage.setItem('admin-certificates-authenticated', 'true');
    } else {
  ...
  useState(() => {
    const isAuth = sessionStorage.getItem('admin-certificates-authenticated');
    if (isAuth === 'true') {
      setIsAuthenticated(true);
    }
  });
      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-purple-900/20 to-blue-900/20 flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <Card className="w-full max-w-md glassmorphism border-white/20 backdrop-blur-sm relative z-10">
          <CardHeader className="text-center pb-6">
            ...
            <CardTitle className="text-white text-2xl">Certificate Admin Access</CardTitle>
            <p className="text-white/60 text-sm">Enter password to access certificate management system</p>
          </CardHeader>
          ...
          <CardContent className="px-6 pb-6">
            ...
            <div className="mt-6 p-4 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 rounded-xl border border-yellow-500/30">
              <p className="text-yellow-400 text-sm text-center">
                <strong>Certificate Management System</strong><br />
                Upload and manage participant certificates
              </p>
            </div>
          </CardContent>
        </Card>
