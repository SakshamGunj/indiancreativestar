...
const IndexV2 = ({ onRegistrationClick }: IndexV2Props) => {
  ...
  const getOrCreateExternalId = () => {
    ...
    if (!externalId) {
      ...
      const random1 = Math.random().toString(36).substr(2, 12); // 12 chars
      const random2 = Math.random().toString(36).substr(2, 12); // 12 chars
  ...
  // 🎯 CUSTOM PAGE VIEW TRACKING - Fires when /indiancreativestar/v2 loads
  useEffect(() => {
    ...
    window.dataLayer.push({
      event: 'page_view_custom',              // ✅ Custom trigger name for GTM
  ...
  useEffect(() => {
    const banner = document.getElementById('mobile-sticky-banner');
    if (banner) {
      if (showRegistrationDrawer || showRegistrationModal) {
  ...
  const handlePaymentInitiated = useCallback(async (orderId: string, regData: any) => {
    ...
    try {
      ...
      if (verifyData.success && verifyData.data.is_paid) {
        ...
        alert('Payment verification failed. Order ID: ' + orderId);
      }
    } catch (error) {
      setIsVerifyingPayment(false);
      alert('Error verifying payment. Order ID: ' + orderId);
    }
  }, [navigate]);

  // Helper function to verify payment without registration data
  ...
  // Old handleFormSubmit removed - now using RegistrationFlowModal with payment integration
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-black" style={{ scrollBehavior: 'smooth' }}>
      ...
      {/* New Drawer-Style Registration Modal */}
      <RegistrationDrawer
      ...
      {isVerifyingPayment && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-md">
          <div className="bg-white rounded-3xl p-8 sm:p-12 max-w-md mx-4 text-center shadow-2xl">
            <div className="mb-6">
              ...
              {paymentVerificationData && (
                <p className="text-sm text-gray-500">Order ID: {paymentVerificationData.orderId}</p>
              )}
            </div>
            <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500 mt-6">This may take a few moments. Please don't close this page.</p>
          </div>
        </div>
      )}
