...
export function EarlyBirdBanner() {
  ...
  return (
    <Card className={`fixed bottom-4 ${isMobile ? 'left-4 right-4' : 'right-4'} z-40 w-full ${isMobile ? 'max-w-full' : 'max-w-md'} bg-white border border-gray-200 shadow-lg p-3 sm:p-4 animate-bounce-in`}>
      <div className="flex justify-between items-start">
        ...
        <button 
          onClick={handleClose}
          className="text-gray-500 hover:text-gray-800 h-6 w-6 flex items-center justify-center touch-target"
        >
          &times;
        </button>
      </div>
      <h3 className="text-base sm:text-lg font-bold mt-2 text-gray-800">Early Bird Discount Ending Soon!</h3>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 mt-2 sm:mt-3">
        ...
        <p className="text-xs text-gray-500">Offer expires in 24 hours</p>
      </div>
    </Card>
  );
}
